...
from transformers import RobertaTokenizerFast, TFRobertaForSequenceClassification, RobertaConfig, TFRobertaModel
...
if __name__ == '__main__':
    ...
    robert = transformer_model.layers[0]
    ...
    robert_model = robert(inputs)[1]
    dropout = Dropout(config.hidden_dropout_prob, name='pooled_output')
    pooled_output = dropout(robert_model, training=False)

    overall = Dense(1, kernel_initializer=TruncatedNormal(stddev=config.initializer_range),
                    name="Overall")(pooled_output)
    time = Dense(1, kernel_initializer=TruncatedNormal(stddev=config.initializer_range),
                 name="Time")(pooled_output)
    geography = Dense(1, kernel_initializer=TruncatedNormal(stddev=config.initializer_range),
    ...
    model = Model(inputs=inputs, outputs=outputs, name='roBERT_MultiLabel')
    model.summary()
    ...
    result_output.to_csv('result/multi_output_robert.csv')
